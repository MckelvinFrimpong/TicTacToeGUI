...
public class TicTacToeFrame extends JFrame{
    ...
    private void createTopPanel()
    {
        ...
        titleLbl.setFont(new Font("Oswald", Font.BOLD, 38));
                            ...
                            if(source instanceof JButton)
                            {
                                ...
                                if(buttonSource.getText().equals(" "))
                                {
                                    ...
                                    if(moveCnt >= MOVES_FOR_TIE)
                                    {
                                        if(isTie())
                                        {
                                            int a = JOptionPane.showConfirmDialog(frame, "It's a tie! Would you like to play again?");
                                            if(a == JOptionPane.YES_OPTION)
                                            {
                                                clearBoard();
                                            }
                                ...
                                {
                                    JOptionPane.showMessageDialog(frame, "Invalid move: spot already taken");
                                }
                            }
                        });
            }
    ...
    private boolean isTie()
    {
        ...
        for(int col=0; col < 3; col++)
        {
            ...
            {
                xFlag = true; // there is an X in this column
            }
            if(board[0][col].getText().equals("O") ||
                    board[1][col].getText().equals("O") ||
                    board[2][col].getText().equals("O"))
            {
                oFlag = true; // there is an O in this column
            }

            if(! (xFlag && oFlag) )
            {
                return false; // No tie can still have a column win
            }
        }
